...
12. **Then implement**, and finish with the browser proof required by `frontend.md`: spawn `feature-verifier` for a flow (verdict must be PASS) or a `bun run dev` walk noted in the run note for a small surface. Compare the render against the locked reference and fix actionable drift — research is not proof. End with [`/verify-done`](verify-done.md).
