...
12. **Build the craft, not just the structure.** Follow [`.brain/recipes/add-premium-surface.md`](../../.brain/recipes/add-premium-surface.md) step 4 — show the product as a real panel, three surface levels, depth as four details (hairline border · 1px inner highlight · one wide soft shadow · one ambient glow), two real type faces, one accent spent sparingly, motion at the subject's real cadence, and one structural memorable move. A structurally correct page with no craft is the "passed every check and still boring" failure.
