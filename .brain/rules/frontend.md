...
| Anything touching color, type, radius, elevation | `app/app.css` tokens + [`../codebase/design-system.md`](../codebase/design-system.md) when its `Lock status` is `ACTIVE` | Committed visual language. **Wins over both tools.** Tokens always win; the *direction* only binds while the lock is active — see the lock bullet under tier 2. |
...
- **Derive the lock before researching.** [`../codebase/design-system.md`](../codebase/design-system.md) opens with a `Lock status` rule, and the status is *derived* rather than written down — `ACTIVE` only when **both** `package.json` `name` and the `origin` remote are still the starter's, `NONE` if either disagrees (exact command in that file's `Lock status` section). A hardcoded status would survive the wholesale `.brain/` copy that `/new-app` performs; neither signal does, and requiring both closes the window where an app has been created but `bun setup` has not run yet. `ACTIVE` (the starter itself) means the public surface already has a direction — Cursor + Linear — and extending that surface is research *within* it; only a deliberate redesign re-opens the lock, and that updates `design-system.md` in the same PR. `NONE` (an app forked from the starter, before it has done its own research) means there is **no** direction to work within: the file is a worked example, not a spec, and the first tier-2 surface runs the full research and rewrites it. Never build toward the starter's monochrome dev-tool direction just because the file is present in the repo — that is inherited furniture, not a decision anyone made about your product.
